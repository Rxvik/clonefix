...
const init = async () => {
    const trending = await fetchJSON(`https://api.tvmaze.com/shows?page=1`);
...
const wireSearch = () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const results = await fetchJSON(`${API}${encodeURIComponent(movie)}`);
            const shows = results.map(r => r.show);
            rowsContainer.innerHTML = '';
            renderRow(`Resultados para ${escapeHTML(movie)}`, shows);
        } catch (err) {
...
const posterCard = (show) => {
    ...
    const img = show?.image?.medium || 'https://via.placeholder.com/600x400?text=No+Imagen';
...
const renderHero = show => {
    if (!show) return;
    const bg = show?.image?.original || show?.image?.medium || 'https://via.placeholder.com/600x400?text=No+Imagen';
...
const openDetail = async (id) => {
    ...
    try {
        ...
        modalBody.innerHTML = `
            <div class="row g-4">
                <div class="col-md-4">
                    <img class="img-fluid rounded" src="${show?.image?.original || show?.image?.medium || 'https://via.placeholder.com/600x400?text=No+Imagen'}"/>
                </div>
                <div class="col-md-8">
                    <div class="mb-2">
                        ${(show.genres || []).map(g => `
                    ...
                    <p class="text-secondary small">
                        ${show.summary || 'Sin sinopsis disponible'}
                    </p>
                    <p class="text-secondary small">
                        ⭐ ${show?.rating?.average ?? "N/A"} • Lenguaje: ${show?.language ?? "N/A"} • Estado: ${show?.status ?? "N/A"} 
                    </p>
        ...
        modalBody.innerHTML = "No se pudo cargar el detalle.";
        console.error(err);
        modal.show();
    }
};
